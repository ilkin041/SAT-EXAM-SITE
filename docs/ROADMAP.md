...
| T3.4 | Hero rebuild: demo folded into the hero as the signature (and the page's one gradient, off the CTA), student-first Appendix B copy minus the unbacked "adaptive", `MockTestCard` + green-check items deleted, answer-bubble lattice, `.hero-rise` 60ms stagger | |
| T3.5 | `ScreenshotTabs` | |
