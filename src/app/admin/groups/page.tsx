      ...
      <div className="mb-8 rounded-2xl border border-border/80 bg-card p-5 shadow-sm">
        ...
        <form action={createGroup} className="flex flex-col gap-3 sm:flex-row">
          <input
            ...
            className="flex-1 rounded-xl border border-input/80 bg-card px-3 py-2 text-body focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring"
          />
          <input
            type="text"
            name="description"
            ...
            <tbody className="divide-y divide-border/60">
              {groups.map((group) => (
                <tr key={group.id} className="transition-colors hover:bg-muted/30">
                  ...
                  <td className="px-6 py-4 text-center tabular text-foreground font-semibold">
                    {group._count.users}
                  </td>
                  <td className="px-6 py-4 text-center tabular text-foreground font-semibold">
                    {group._count.tests}
